  ...
  async initialize() {
    ...
    try {
      ...
      const prompt = ChatPromptTemplate.fromMessages([
        ["system", `You are a helpful assistant for a sushi restaurant. You help customers find menu items, answer questions about the menu, and make recommendations.
...
If you can't find what the user is looking for, politely suggest alternatives.`],
      ...
      this.executor = new AgentExecutor({
        agent: this.agent,
        tools: this.tools,
        verbose: process.env.ENABLE_PERFORMANCE_LOGGING === 'true', // Show tool usage in logs
        maxIterations: 5,
        returnIntermediateSteps: true
      });

      this.initialized = true;
      console.log('✅ Agent service initialized with', this.tools.length, 'tools');
    } catch (error) {
      console.error('❌ Agent service initialization failed:', error.message);
      this.initialized = false;
    }
  }
    ...
    return [
      ...
      new DynamicTool({
        ...
        func: async (priceRangeJson) => {
          try {
            console.log(`🔧 Tool: filter_by_price(${priceRangeJson})`);
