...
# Constants
# ---------------------------------------------------------------------------
# Load .env from the MCP project directory (same folder as server.py)
...
def _new_vectorstore() -> Chroma:
    """Create a fresh vectorstore handle to avoid stale collection references."""
...
async def app_lifespan(app):
    ...
    # Avoid hard crash on startup if Chroma is temporarily unavailable.
    # Tool/resource handlers already catch exceptions and return clear errors.
    vectorstore = None
    qa_chain = None
    try:
        http_client = chromadb.HttpClient(host=CHROMA_HOST, port=CHROMA_PORT)
        ...
        print(
            f"Warning: Chroma connection unavailable at startup ({type(e).__name__}: {e})",
            file=sys.stderr,
        )
...
async def sushi_rag_query(params: SushiQueryInput, ctx: Context) -> str:
    ...
    ctx.info("Retrieving relevant documents...")

    try:
        vectorstore = _new_vectorstore()
        llm         = ctx.request_context.lifespan_context["llm"]

        # Retrieve a broader set, then apply deterministic constraints (price/category/spice)
        docs_and_scores = vectorstore.similarity_search_with_relevance_scores(
            params.question, k=max(params.k_docs * 3, 10)
        )
        constraints = _infer_constraints(params.question)
        constrained = _apply_constraints_docs(docs_and_scores, constraints)
        has_constraints = _has_constraints(constraints)
        final_docs_and_scores = constrained if has_constraints else docs_and_scores
        source_docs = [doc for doc, _ in final_docs_and_scores[:params.k_docs]]

        if has_constraints:
            if len(source_docs) == 0:
                return f"I couldn't find items matching '{params.question}' with the requested constraints."

            # Deterministic ordering for constrained queries
            def _price(doc):
                try:
                    return float((doc.metadata or {}).get("price", 0))
                except Exception:
                    return 0.0
            source_docs = sorted(source_docs, key=lambda d: (_price(d), (d.metadata or {}).get("name", "")))
                    ...
                    "results": [
                        {
                            ...
                            "price": (d.metadata or {}).get("price"),
                            "category": (d.metadata or {}).get("category"),
                            "spiceLevel": (d.metadata or {}).get("spiceLevel", 0),
                            "dietary": (d.metadata or {}).get("dietary", ""),
                        }
                        for d in source_docs
                    ],
                }, indent=2)

            lines = ["Here are the matching menu items:\n"]
            for i, d in enumerate(source_docs, 1):
                m = d.metadata or {}
                spice = int(m.get("spiceLevel", 0) or 0)
                spice_text = f", spice {spice}/3" if spice > 0 else ""
                cat = m.get("category")
                cat_text = f" ({cat}{spice_text})" if cat else ""
                lines.append(f"{i}. {m.get('name', 'Unknown')} - ${m.get('price', 'N/A')}{cat_text}")
            if params.include_sources:
                lines.append(_format_sources(source_docs, params.response_format))
            return "\n".join(lines)

        await ctx.report_progress(40, 100)
        ctx.info("Generating answer...")
        prompt = ChatPromptTemplate.from_messages([
            ("system", RAG_SYSTEM_PROMPT),
            ("human", "{question}"),
        ])
...
async def sushi_semantic_search(params: SushiSearchInput, ctx: Context) -> str:
    ...
    ctx.info("Running semantic search...")

    try:
        vectorstore: Chroma = _new_vectorstore()
...
async def sushi_list_topics(params: ListTopicsInput, ctx: Context) -> str:
    ...
    try:
        ...
        for meta in results.get("metadatas", []):
            # Support both document-style metadata and menu-item metadata.
            # Existing precedence for source/title/topic is preserved.
            label = (
                meta.get("source")
                or meta.get("title")
                or meta.get("topic")
            ...
            if label:
                topics.add(str(label))

            # Optionally include dietary tags (stored as comma-separated string in this app).
            dietary_raw = meta.get("dietary")
            if dietary_raw:
                if isinstance(dietary_raw, str):
                    dietary_parts = [d.strip() for d in dietary_raw.split(",") if d.strip()]
                elif isinstance(dietary_raw, (list, tuple, set)):
                    dietary_parts = [str(d).strip() for d in dietary_raw if str(d).strip()]
                else:
                    dietary_parts = [str(dietary_raw).strip()]
                for d in dietary_parts:
                    topics.add(f"dietary:{d}")
...
if __name__ == "__main__":
    ...
    if args.http:
        print(f"Starting Sushi RAG MCP Server (HTTP) on port {args.port}...", file=sys.stderr)
        import uvicorn
        ...
        print("Starting Sushi RAG MCP Server (stdio)...", file=sys.stderr)
        mcp.run()
